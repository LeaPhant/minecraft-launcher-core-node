import { NearestFilter, LinearFilter, DoubleSide } from 'three/src/constants';
...
import { BufferAttribute as BufferAttribute$1, Vector2 as Vector2$1 } from 'three';
...
class BlockModelObject extends Object3D {
    constructor() {
        super(...arguments);
...
class BlockModelFactory {
    ...
    async getObject(model, options) {
        ...
        for (const element of model.elements) {
            ...
            blockGeometry.setAttribute("uv", new BufferAttribute(new Float32Array(uvAttr.length * 2), 2).copyVector2sArray(uvAttr));
            /**
             * bake rotation start
             */
            if (element.rotation) {
...
BlockModelFactory.TRANSPARENT_MATERIAL = new MeshBasicMaterial({ transparent: true, opacity: 0, alphaTest: 0.5 });
...
function mapCubeUV(mesh, src) {
    ...
    function mapUV(x1, y1, x2, y2) {
        ...
        y1 = 1 - (y1 * tileUvH);
        y2 = 1 - (y2 * tileUvH);
        uvs.push(new Vector2$1(x1, y1), new Vector2$1(x2, y1), new Vector2$1(x1, y2), new Vector2$1(x2, y2));
    }
    ...
    const attr = new BufferAttribute$1(new Float32Array(uvs.length * 2), 2).copyVector2sArray(uvs);
    mesh.geometry.setAttribute("uv", attr);
}
class PlayerObject3D extends Object3D {
...
export { BUILTIN_GENERATED, BlockModelFactory, BlockModelObject, DEFAULT_DISPLAY, DEFAULT_TRANSFORM, PlayerModel, PlayerObject3D };
